...
async function suggest(request, h) {
  ...
  const title =
    (request.payload.new
      ? `Add '${company.name || company.web}'`
      : `Update '${company.name || company.slug}'`) +
...
**[Edit](https://company-json.datenanfragen.de/#!doc=${
...
**[Edit](https://company-json.datenanfragen.de/#!doc=${
        ...
        .catch((e) => {
          console.error(e);
          return h.response({ message: "Bad Gateway. Failed to create issue." })
            .code(502);
        });
    });
}

function commitStringFileToPullRequest(
  owner,
  repo,
  pr,
  path,
